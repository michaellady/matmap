  ...
  return (
    <Pressable style={styles.card} onPress={onPress}>
      ...
      <View style={styles.techniques}>
        <TechniqueRow label="Standing" name={classLog.standing_name} />
        <TechniqueRow label="Guard" name={classLog.guard_name} />
        {classLog.pinning_name && (
          <TechniqueRow label="Pinning" name={classLog.pinning_name} />
        )}
...
function TechniqueRow({ label, name }: { label: string; name: string }) {
  return (
    <View style={styles.techniqueRow}>
      ...
      <Text style={styles.techniqueName} numberOfLines={1}>{name}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  card: {
    ...
    marginHorizontal: 16, marginVertical: 6, minHeight: 44,
    borderWidth: 1, borderColor: DARK_THEME.border,
  },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  date: { fontSize: 16, fontWeight: '700', color: DARK_THEME.text },
  techniques: { gap: 6 },
  techniqueRow: { flexDirection: 'row', gap: 8 },
  techniqueLabel: { fontSize: 13, color: DARK_THEME.textMuted, width: 72 },
  techniqueName: { fontSize: 13, color: DARK_THEME.textSecondary, flex: 1 },
});
